...
class Renderer:
    ...
    def preview(
        ...
        ``layout``:
          * ``"split"`` (default) — split the startup screen's 3D viewport
            vertically: LEFT = camera view + Rendered (the "monitor"),
            RIGHT = free-orbit perspective + Solid (where you grab / rotate
            mirrors). The reflection in the left half updates live when you
            operate in the right half.
          * ``"single"`` — every 3D area is camera view + Rendered. Simpler;
            you lose the free-orbit area.
    ...
    def _do_split():
        try:
            ...
            for s in right.spaces:
                _configure_space(s, "PERSP", "SOLID")
            print("[VMirror preview] split layout applied")
        except Exception as e:
            print("[VMirror preview] split failed:", e)
...
def _schedule_triple_layout():
    """Left column: L-mirror camera (top) + R-mirror camera (bottom), both
    Rendered. Right column: free-orbit perspective, Solid shading. Uses a
    multi-tick state machine — Blender needs a redraw between area_split
    calls before the resulting areas' dimensions are valid to act on."""
    ...
    def _do():
        try:
            ...
            if state["stage"] == 2:
                ...
                print("[VMirror triple] layout applied (3 panes)", flush=True)
                return None
        except Exception as e:
            import traceback
...
def _build():
    ...
    if payload.get("preview"):
        ...
        if layout == "split":
            report["preview"] = _schedule_split_layout()
        elif layout == "triple":
            report["preview"] = _schedule_triple_layout()
        else:
